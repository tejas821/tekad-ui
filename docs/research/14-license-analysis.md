...
Work infringes terminates *your* patent licences (copyright grant survives).
§4 four conditions: supply the licence; **mark modified files**; retain
notices in Source form of derivatives; reproduce the NOTICE file *if the
licensor ships one*. §6 **trademark exclusion** — the licence does not grant
...
- **Zero new compliance cost for consumers.** `rxjs` — a mandatory peer of
  every Angular app — is *already* Apache-2.0. Every Angular application on
...
- **GPL/LGPL/AGPL** — an Angular library is compiled and tree-shaken into the
  ...
  AGPL §13 is worse, since network-served software *is* the deployment target.
...
*stays* Apache-2.0 and carries §4's conditions. An app that merely
...
counter-project formed in response. In August 2026, a visible CLA on a *new*
