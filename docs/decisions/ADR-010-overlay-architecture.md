...
**Measured.** Chromium (build 1194) passes every scenario with _both_ exit
...
viewport — but from ~300 ms its pixels paint _beneath_ a max-z-index cover.
...
marks it _Limited availability, not Baseline_.
...
1. Gecko runtime behaviour remains unverified. Because the chosen strategy does
   ...
   architecture — it _would_ have blocked the native-only or hybrid options.
...
`aria-expanded` must fire on _close intent_, not on animation end — otherwise
...
- **WebKitGTK 2.52.3** (_WebKit evidence only; does not certify Safari_) —
...
run. Case 9 (**PASS**) proves _reconciliation without leaks_ — it is **not**
...
_intent_, not animation end; exit duration read from computed style for the
...
`_hardReset()` originally set `state = 'closed'` _after_ `hidePopover()`, so a
