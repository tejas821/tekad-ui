...
zero entries *including for the control*, so no conclusion may be drawn either
...
Spike A proved the *consumption* end (tsserver offers the deep import).
Spike B measures the *production* end, with a real ng-packagr 22 build.
...
unimportable by default, confirming the property the CI assertion must *keep*
...
shared-chunk splitting. Accurate: ng-packagr emits no shared *chunk files*, but
...
*achievable* — a different claim from *satisfied*.
